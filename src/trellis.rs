...
/// Value-sorted `(code value, window)` pairs, the beam encoder's free-start seed table.
fn sorted_code(code: &[f32]) -> Vec<(f32, u32)> {
    let mut cs: Vec<(f32, u32)> = code
        .iter()
        .enumerate()
        .map(|(w, &val)| (val, w as u32))
        .collect();
    // total_cmp: the table is finite by construction, but a total order costs nothing.
    cs.sort_by(|a, b| a.0.total_cmp(&b.0));
    cs
}

/// Every knob a deployment sets, resolved; the constructor core takes this so the
/// environment path and the configuration path assemble the same struct.
struct Knobs {
    mem: u8,
    beam: usize,
    resid_m: usize,
...
/// A declared operating point for [`TrellisQuantizer::from_config`]: the knobs a
/// deployment fixes, none read from the environment. Everything the research harness
/// exposes as an ablation (beam, residual sketch, anisotropic metric, tail-biting,
/// shortlist, 1MAD code, vector trellis) stays at its evaluated default here.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct TrellisConfig {
    /// Source dimension; padded to a power of two by the caller's loader, as the
    /// harness does, so the rotation's single full-length block applies.
    pub dim: usize,
    /// Emission bits per coordinate.
    pub bits: u8,
    /// Trellis memory `M`: the state is the last `M` emitted bits, the reconstruction
    /// table has `2^(M+bits)` entries indexed by the `(M+bits)`-bit window, and the
    /// record stores `ceil(start_bits/8)` start bytes.
    pub mem: u8,
    /// Seed of the randomized Hadamard rotation shared by every codec in a comparison.
    pub rotation_seed: u64,
    /// Rounds of seeded sign flip + FWHT; three is the evaluated operating point.
    pub rotation_rounds: usize,
    /// Bits of the start state the encoder may choose from and the record stores
    /// (`mem` = full free start, the evaluated mode; `0` = fixed start).
    pub start_bits: u8,
    /// Width of the stored rescale scalar; `0` keeps the exact f32.
    pub rescale_bits: u8,
    /// Codes-only scoring (no fp32 reconstruction cache resident).
    pub codes_only: bool,
}

impl TrellisConfig {
    /// The evaluated operating point at `(dim, bits, mem)`: seed 42, three rotation
    /// rounds, full free start, exact f32 rescale, reconstruction cache resident.
    pub fn new(dim: usize, bits: u8, mem: u8) -> Self {
        Self {
            dim,
            bits,
            mem,
...
impl TrellisQuantizer {
    ...
    /// The evaluated codec at a declared operating point, with no environment read.
    ///
    /// `new` is the research-harness entry point: it reads every `ULTRAVEC_*` knob so a
    /// tier can sweep without a recompile. A consumer that must be deterministic in any
    /// environment -- the KV-cache artifact, a kernel table generator -- goes through
    /// here instead. Both paths share [`TrellisQuantizer::build`], so a quantizer built
    /// from a configuration that mirrors the environment is the same quantizer
    /// bit for bit (pinned by a test).
    pub fn from_config(cfg: &TrellisConfig) -> Self {
        assert!(cfg.mem >= 1, "trellis memory must be at least one bit");
        Self::build(
            cfg.dim,
            cfg.bits,
            Knobs {
                ...
                resid_m: 0,
                branch_lambda: 0.0,
                codes_only: cfg.codes_only,
                fixed_start: false,
                tail_biting: false,
                tailbite_candidates: 1,
                shortlist_c: 0,
                shortlist_asym: false,
                shortlist_rnorm: true,
                use_mad: false,
                rescale_bits: cfg.rescale_bits,
                start_bits: cfg.start_bits.min(cfg.mem),
                v: 1,
                rotation: Rotor::new_oblivious_with_rounds(
                    cfg.dim,
                    cfg.rotation_seed,
                    cfg.rotation_rounds,
                ),
    ...
    /// Construct from resolved knobs. The one place the code table, the beam seed
    /// table and the struct are assembled, so the environment path and the
    /// configuration path cannot drift apart.
    fn build(dim: usize, bits: u8, k: Knobs) -> Self {
        let Knobs {
            mem,
            beam,
            resid_m,
    ...
    /// The reconstruction table as f32 bit patterns: `2^(mem+bits)` entries indexed by
    /// the `(mem+bits)`-bit window `(state << bits) | emission`, normalized to zero
    /// mean and unit variance over the table. A kernel embeds exactly these bits so
    /// its decode is byte-exact against this codec. (Scalar trellis, `V = 1`.)
    pub fn code_table_bits(&self) -> Vec<u32> {
        assert_eq!(
            self.v, 1,
            "code_table_bits is defined for the scalar trellis"
        );
        self.code.iter().map(|c| c.to_bits()).collect()
    }

    /// Trellis memory `M` in bits.
    pub fn memory_bits(&self) -> u8 {
        self.mem
    }

    /// The rotation's ±1 sign rounds, in application order (oblivious Hadamard only).
    pub fn rotation_sign_rounds(&self) -> &[Vec<f32>] {
        self.rotation
            .sign_rounds()
            .expect("the oblivious codec rotates with a seeded Hadamard")
    }

    /// Encode one row to its record: the packed emissions (`ceil(dim*bits/8)` bytes)
    /// and the chosen start state (`start_bits` wide). The row is normalized and
    /// rotated inside, as `add` does; the direction is what the codes carry.
    pub fn encode_codes(&self, row: &[f32]) -> (Vec<u8>, u32) {
        let tv = self.encode(row);
        (tv.codes, tv.start)
    }

    /// The forward rotation applied to a unit row (the `u_r` the encoder standardizes).
    /// A consumer whose wire contract fixes its own normalization arithmetic feeds the
    /// result of that normalization here, then [`TrellisQuantizer::encode_rotated`].
    pub fn rotate(&self, unit: &[f32]) -> Vec<f32> {
        self.rotation.apply(unit)
    }

    /// Encode an already-rotated unit row: standardize by its own `std_about_mean`
    /// (floored at `f32::EPSILON`) and run the exact free-start Viterbi. This is
    /// `encode_codes` with the caller's normalization and this codec's rotation --
    /// the same standardization, tie rule, table and start selection. Scalar trellis,
    /// exact Viterbi, no tail-biting.
    pub fn encode_rotated(&self, u_r: &[f32]) -> (Vec<u8>, u32) {
        assert_eq!(
            self.v, 1,
            "encode_rotated is defined for the scalar trellis"
        );
        assert_eq!(u_r.len(), self.dim, "row must have the trellis dimension");
        ...
        (pack_indices(&emissions, self.b_step), start)
    }

    /// Decode a record to the raw code vector in source coordinates: the `dim` table
    /// values selected by the windows, inverse-rotated, NOT normalized. A consumer
    /// scales it by its own stored gain (a norm ratio, or the rescaled-estimator
    /// scalar). Exactly the reconstruction `reconstruct_unit` normalizes.
    pub fn decode_codes(&self, codes: &[u8], start: u32) -> Vec<f32> {
        assert_eq!(self.v, 1, "decode_codes is defined for the scalar trellis");
        let tv = TVec {
            codes: codes.to_vec(),
            aux: None,
            rescale: 1.0,
        ...
        self.decode_into(&tv, &mut scratch, &mut xbar);
        self.rotation.apply_inverse(&xbar)
    }

    /// The `(mem+bits)`-bit window that selects coordinate `i`'s reconstruction: the
    /// field at bit offset `i*bits` of the stream `[start (mem bits) ‖ codes]`, read
    /// MSB-first. This is the property the systems section rests on -- a window is a
    /// function of the record bytes at a known offset, not of a walk from the start
    /// -- so a decoder may gather any subset of coordinates in any order.
    pub fn window_at(&self, codes: &[u8], start: u32, i: usize) -> u32 {
        assert_eq!(self.v, 1, "window_at is defined for the scalar trellis");
        let width = self.mem as u32 + self.b_step as u32;
        let wmask = (1u64 << width) - 1;
        let mut acc = (start as u64) & ((1u64 << self.mem) - 1);
        let end_bit = (i as u32 + 1) * self.b_step as u32; // bits of codes consumed
                                                           // Shift in whole bytes until the window's last bit is in, then align.
        let mut consumed = 0u32;
        let mut byte = 0usize;
        while consumed < end_bit {
            acc = (acc << 8) | codes[byte] as u64;
            byte += 1;
            consumed += 8;
        }
        let extra = consumed - end_bit; // bits past the window's end
        ((acc >> extra) & wmask) as u32
    }
    ...
    /// Beam width for the beam encoder (0 => exact Viterbi, the evaluated mode). The
    /// beam is a measured dead end for quality (Appendix B.1) and exists as an
    /// encode-time concession; this builder lets tests reach it without the
    /// `ULTRAVEC_TRELLIS_BEAM` environment read.
    pub fn with_beam(mut self, width: usize) -> Self {
        self.beam = width;
        // The beam seeds its first step from the value-sorted code table, which the
        // constructor builds only when the beam is on. A builder that turned the beam
        // on without it would seed nothing and panic on the first row.
        let need_sorted = width > 0 && self.v == 1;
        if need_sorted && self.code_sorted.is_empty() {
            self.code_sorted = std::sync::Arc::new(sorted_code(&self.code));
        } else if !need_sorted {
            self.code_sorted = std::sync::Arc::new(Vec::new());
        }
        self
    }
    ...
    fn encode_beam(&self, o: &[f32]) -> TVec {
        ...
        for &xi in u_rs.iter().skip(1) {
            ...
            if cand.len() > beam {
                cand.select_nth_unstable_by(beam, |a, b| a.0.total_cmp(&b.0));
                cand.truncate(beam);
            }
            live.clear();
        ...
        // `total_cmp`, as in the exact path: a NaN cost from a degenerate row must
        // yield a garbage-but-valid code, never a panic that takes the batch down.
        let best = live.iter().min_by(|a, b| a.0.total_cmp(&b.0)).unwrap();
        let start = best.3;
...
mod tests {
    ...
    /// A NaN or all-zero input row must yield a valid-length record from BOTH encoders,
    /// never a panic. The exact path was hardened to `total_cmp` first; the beam path's
    /// candidate selection and final `min_by` kept `partial_cmp().unwrap()` and would
    /// panic the whole batch on the first degenerate row. A panic here is the defect.
    /// The configuration path is the environment path: at the operating point the
    /// tiers declare (seed 42, three rounds, full free start, exact rescale), a
    /// quantizer from `TrellisConfig::new` must reproduce `new()` byte for byte.
    /// The check tier strips `ULTRAVEC_*`, so `new()` here is the documented default.
    #[test]
    fn from_config_reproduces_new_bit_for_bit() {
        let dim = 128;
        let bits = 2;
        let rows = rand_unit(48, dim, 11);
        let via_env = TrellisQuantizer::new_with_overrides(dim, bits, Some(6), None);
        let via_cfg = TrellisQuantizer::from_config(&TrellisConfig::new(dim, bits, 6));
        assert_eq!(via_env.code_table_bits(), via_cfg.code_table_bits());
        for row in &rows {
            assert_eq!(via_env.encode_codes(row), via_cfg.encode_codes(row));
        }
    }

    /// `window_at` reads the window from the record bytes at a known offset; it must
    /// equal the window the state chain reaches at every coordinate.
    #[test]
    fn window_at_matches_the_state_chain() {
        for (dim, bits, mem) in [(64usize, 2u8, 6u8), (128, 3, 6), (128, 2, 10), (128, 1, 12)] {
            let q = TrellisQuantizer::from_config(&TrellisConfig::new(dim, bits, mem));
            for row in rand_unit(4, dim, 5) {
                let (codes, start) = q.encode_codes(&row);
                let mut scratch = vec![0u16; dim];
                unpack_indices(&codes, bits, &mut scratch);
                let mut state = start;
                for (i, &e) in scratch.iter().enumerate() {
                    let w = q.window(state, e as u32);
                    assert_eq!(
                        q.window_at(&codes, start, i),
                        w,
                        "dim {dim} bits {bits} M {mem} i {i}"
                    );
    ...
    /// `encode_rotated` fed the codec's own normalization and rotation reproduces
    /// `encode_codes` bit for bit: the split exposes the pipeline, it does not fork it.
    #[test]
    fn encode_rotated_reproduces_encode_codes() {
        let dim = 128;
        let q = TrellisQuantizer::from_config(&TrellisConfig::new(dim, 3, 6));
        for row in rand_unit(16, dim, 9) {
            let norm = l2_norm(&row).max(f32::EPSILON);
            let unit: Vec<f32> = row.iter().map(|x| x / norm).collect();
            let u_r = q.rotate(&unit);
            assert_eq!(q.encode_rotated(&u_r), q.encode_codes(&row));
        }
    }

    /// `decode_codes` normalized is exactly `reconstruct_unit`, so a consumer that
    /// stores its own gain reconstructs what the harness scores.
    #[test]
    fn decode_codes_normalized_is_reconstruct_unit() {
        let dim = 128;
        let q = TrellisQuantizer::from_config(&TrellisConfig::new(dim, 2, 8));
        for row in rand_unit(8, dim, 3) {
            let (codes, start) = q.encode_codes(&row);
            let raw = q.decode_codes(&codes, start);
            let n = l2_norm(&raw);
            let unit: Vec<f32> = raw.iter().map(|x| x / n).collect();
            let expected = q.reconstruct_unit(&row).unwrap();
            for (a, b) in unit.iter().zip(&expected) {
    ...
    #[test]
    fn degenerate_rows_never_panic_in_either_encoder() {
        let dim = 64;
        let mut rows: Vec<Vec<f32>> = rand_unit(2, dim, 7);
        rows.push(vec![f32::NAN; dim]);
        let mut one_nan = rows[0].clone();
        one_nan[3] = f32::NAN;
        rows.push(one_nan);
        rows.push(vec![0.0; dim]);
        rows.push(vec![f32::INFINITY; dim]);
        for beam in [0usize, 8] {
            let q = TrellisQuantizer::new_with_overrides(dim, 2, Some(6), None).with_beam(beam);
            for (i, row) in rows.iter().enumerate() {
                let tv = q.encode(row);
                assert_eq!(
                    tv.codes.len(),
                    dim * 2 / 8,
                    "row {i} beam {beam}: payload length"
                );
                assert!(
                    (tv.start as usize) < (1usize << 6),
                    "row {i} beam {beam}: start state in range"
                );
            }
        }
    }
